...
DWORD scu_mil::pulse1_en_dis(bool onoff)
{
/*	eb_status_t status;

	if(onoff){
		if ((status = eb_device_write(scu_info.device, scu_info.mil_base+lemo_nr, EB_BIG_ENDIAN|EB_DATA32, value, 0, eb_block)) != EB_OK)
        	return device_error;
	}
	else {
	}
...
DWORD scu_mil::pulse2_en_dis(bool onoff)
{
}

// erzeugt einen puls an den jeweiligen ausgaengen
// ---------------------
DWORD scu_mil::puls_set(WORD lemo_nr, WORD value)
{
	eb_status_t status;
...
string scu_mil::scu_milerror(DWORD status)
{
        ...
        case mil_ifknr_error:
                ...
                break;
        case device_error:
                ErrorMessage = "Send/read to device failure";
                break;
	}
	return ErrorMessage;
}
...
DWORD scu_mil::scu_milbus_puls1_start(DWORD &errorstatus)
{
	return status_ok;
}

DWORD scu_mil::scu_milbus_puls1_stop(DWORD &errorstatus)
{
	return status_ok;
}

DWORD scu_mil::scu_milbus_puls2_start(DWORD &errorstatus)
{
	return status_ok;
}

DWORD scu_mil::scu_milbus_puls2_stop(DWORD &errorstatus)
{
	return status_ok;
}
